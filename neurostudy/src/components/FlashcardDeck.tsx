...
export default function Home() {
  ...
  const handleSubmit = async ({
    url, file, format: fmt, microMode, pasteText,
  }: { url: string; file: File | null; format: Format; microMode: boolean; pasteText?: string }) => {
    setLoading(true);
    setError('');
    setResults(null);
    setFormat(fmt);
    ...
    try {
      ...
      if (pasteText) {
        // Skip ingest for paste - send directly to synthesize
        rawText = pasteText;
          ...
          <div style={{ width: '100%', maxWidth: 640 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
              ...
              <button
                type="button"
                onClick={() => {
                  results && downloadMarkdown(results);
